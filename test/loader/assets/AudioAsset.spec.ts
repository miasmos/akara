...
describe('AudioAsset', () => {
    it('should accept a name parameter', () => {
        const result = new AudioAsset('test', 'test1');
    ...
    describe('load event', () => {
        it('should emit an event when loading is complete', done => {
            ...
            result.load();
            result.getRef().dispatchEvent(new Event('canplaythrough'));
        }).timeout(10);
    ...
    describe('equals()', () => {
        it('should return true given an asset with the same name and type', () => {
            const result = new AudioAsset('test', 'test1');
            const result1 = result.equals(new AudioAsset('test', 'test1'));
    ...
    describe('getRef()', () => {
        it('should return the ref type', () => {
            const result = new AudioAsset('test', 'test1');
            expect(result.getRef() instanceof HTMLAudioElement).to.equal(true);
        });
    });
});
